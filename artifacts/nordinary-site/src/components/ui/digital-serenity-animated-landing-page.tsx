...
const DigitalSerenityBackground = () => {
  ...
  const pageStyles = `
    #mouse-gradient-react {
      ...
      background-image: radial-gradient(circle, rgba(255,233,71, 0.08), rgba(255,233,71, 0.03), transparent 70%);
    ...
    .grid-line { stroke: rgba(255,233,71, 0.3); stroke-width: 0.5; opacity: 0; stroke-dasharray: 5 5; stroke-dashoffset: 1000; animation: grid-draw 2s ease-out forwards; }
    .detail-dot { fill: rgba(255,233,71, 0.5); opacity: 0; animation: pulse-glow 3s ease-in-out infinite; }
    .floating-element-animate { position: fixed; width: 2px; height: 2px; background: rgba(255,233,71, 0.5); border-radius: 50%; opacity: 0; animation: float 4s ease-in-out infinite; animation-play-state: paused; z-index: 0; pointer-events: none;}
    ...
    .ripple-effect { position: fixed; width: 4px; height: 4px; background: rgba(255,233,71, 0.6); border-radius: 50%; transform: translate(-50%, -50%); pointer-events: none; animation: pulse-glow 1s ease-out forwards; z-index: 9999; }
  `;

  return (
    <>
      <style>{pageStyles}</style>
      <div className="fixed inset-0 bg-[#080808] pointer-events-none z-[-1] overflow-hidden">
        
        <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <defs>
            <pattern id="gridReactDarkResponsive" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M 60 0 L 0 0 0 60" fill="none" stroke="rgba(255,233,71, 0.05)" strokeWidth="0.5"/>
            </pattern>
          </defs>
...
export default DigitalSerenityBackground;
